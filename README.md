...
- **Per-project installation only** — Plugins are installed under
  `.plain-agent/claude-code-plugins/` and must be installed per project by
  running `plain install-claude-code-plugins` from the project root. Global
  installation (e.g., under `~/.plain-agent`) is not supported. Plugins may
  include skills that the agent invokes autonomously, so scoping them to the
  project keeps approval rules and permission management straightforward.
