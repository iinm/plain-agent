...
minial_dockerfile=examples/minimal/Dockerfile
...
echo "case: run basic command with minimal Dockerfile"
# when/then:
plain-sandbox --dockerfile "$minial_dockerfile" --rebuild echo hello | grep -qE "^hello$"


echo "case: receive stdin"
# when/then:
echo hello | plain-sandbox --dockerfile "$minial_dockerfile" --rebuild cat | grep -qE "^hello$"


echo "case: --dry-run option displays the command that would be executed"
# when:
out=$(plain-sandbox --dry-run --dockerfile "$minial_dockerfile" touch test)
# then:
...
# when:
out=$(plain-sandbox --dry-run --dockerfile "$minial_dockerfile" --platform linux/amd64 true)
# then:
...
# when:
out=$(plain-sandbox --dry-run --dockerfile "$minial_dockerfile" --tty true)
# then:
...
# when:
out=$(plain-sandbox --dry-run --dockerfile "$minial_dockerfile" --no-cache true)
# then:
...
# when:
out=$(env TZ="Asia/Tokyo" plain-sandbox --dry-run --dockerfile "$minial_dockerfile" --no-cache true)
# then:
...
# when:
out=$(env TERM="xterm-256color" plain-sandbox --dry-run --dockerfile "$minial_dockerfile" --no-cache true)
# then:
...
# when:
out=$(plain-sandbox --dry-run --dockerfile "$minial_dockerfile" --env-file .env true)
# then:
grep -qE "DRY_RUN: docker run .+ --env-file .env" <<< "$out"

echo "case: --env option passes environment variables to docker run"
# when:
out=$(plain-sandbox --dry-run --dockerfile "$minial_dockerfile" --env FOO=bar true)
# then:
grep -qE "DRY_RUN: docker run .+ --env FOO=bar" <<< "$out"

echo "case: --env option can be specified multiple times"
# when:
out=$(plain-sandbox --dry-run --dockerfile "$minial_dockerfile" --env FOO=bar --env BAZ=qux true)
# then:
grep -qE "DRY_RUN: docker run .+ --env FOO=bar" <<< "$out"
...
# when:
out=$(plain-sandbox --dry-run --dockerfile "$minial_dockerfile" --volume bin true)
# then:
...
# when:
out=$(plain-sandbox --dry-run --dockerfile "$minial_dockerfile" --volume-each package.json:node_modules true)
# then:
...
# when:
out=$(plain-sandbox --dry-run --dockerfile "$minial_dockerfile" --volume-each node_modules true)
# then:
grep -qE " --mount type=volume,source=plain-sandbox--sandbox-.+--examples-nodejs-monorepo-node_modules,target=/.+/nodejs-monorepo/node_modules,consistency=delegated" <<< "$out"
...
# when:
out=$(plain-sandbox --dry-run --dockerfile "$minial_dockerfile" --mount-readonly bin:/mnt/bin-readonly --mount-writable bin:/mnt/bin-writable true)
# then:
...
# when:
out=$(plain-sandbox --dry-run --dockerfile "$minial_dockerfile" --mount-readonly bin --mount-writable bin true)
# then:
...
# when:
out=$(plain-sandbox --dry-run --dockerfile "$minial_dockerfile" --publish 8000:8000 true)
# then:
...
plain-sandbox --dockerfile "$minial_dockerfile" --rebuild bash -c 'echo $(id -u):$(id -g)' | grep -qE "$(id -u):$(id -g)"
...
# when/then:
plain-sandbox --dockerfile "$minial_dockerfile" --rebuild ls -ld . | grep -qE sandbox


echo "case: working directory is read-only by default"
# when:
out=$(plain-sandbox --dockerfile "$minial_dockerfile" touch test 2>&1) || status=$?
# then:
test "$status" -ne 0
...
# when/then:
plain-sandbox --allow-write --dockerfile "$minial_dockerfile" --rebuild touch test && test -e test
rm -f test
...
nc_pid=$!
# when:
out=$(plain-sandbox --dockerfile "$minial_dockerfile" --rebuild busybox nc -w 2 host.docker.internal 18080 < /dev/null 2>&1) || status=$?
# then:
test "$status" -ne 0
...
nc_pid=$!
# when:
out=$(plain-sandbox --dockerfile "$minial_dockerfile" --rebuild --allow-net host.docker.internal busybox nc -w 2 host.docker.internal 18080 < /dev/null 2>&1) || status=$?
# then:
...
nc_pid=$!
# when:
plain-sandbox --dockerfile "$minial_dockerfile" --rebuild --allow-net host.docker.internal:18080 busybox nc -w 2 host.docker.internal 18080 < /dev/null
# cleanup:
if lsof -i:18080 | grep -q "$nc_pid"; then
  kill "$nc_pid"
fi
...
nc_pid=$!
# when:
plain-sandbox --dockerfile "$minial_dockerfile" --rebuild --allow-net 0.0.0.0/0 busybox nc -w 2 8.8.8.8 443 < /dev/null
# cleanup:
if lsof -i:18080 | grep -q "$nc_pid"; then
  kill "$nc_pid"
fi
...
# when/then:
plain-sandbox --dockerfile "$minial_dockerfile" --rebuild --allow-net example.com dig +time=3 +tries=1 +short example.com | grep -qE "([0-9]{1,3}\.){3}[0-9]{1,3}"


echo "case: --allow-net rejects DNS resolution of non-allowed domains"
# when:
out=$(plain-sandbox --dockerfile "$minial_dockerfile" --rebuild --allow-net example.com dig +time=3 +tries=1 google.com 2>&1) || status=$?
# then:
...
# when:
out=$(plain-sandbox --dockerfile "$minial_dockerfile" --rebuild --allow-net example.com bash -c 'dig +time=3 +tries=1 @127.0.0.11 google.com; dig +time=3 +tries=1 @8.8.8.8 google.com' 2>&1) || status=$?
# then:
test "$status" -ne 0
...
# given:
plain-sandbox --dockerfile "$minial_dockerfile" --rebuild --keep-alive 5 --verbose true &> /dev/null
# when:
out=$(plain-sandbox --dockerfile "$minial_dockerfile" --keep-alive 5 --verbose true 2>&1)
# then:
...
# given:
plain-sandbox --dockerfile "$minial_dockerfile" --verbose true &> /dev/null
# when:
out=$(plain-sandbox --dockerfile "$minial_dockerfile" --rebuild --verbose --dry-run true 2>&1)
# then:
...
plain-sandbox --dockerfile "$minial_dockerfile" --verbose true &> /dev/null
# when:
out=$(plain-sandbox --dockerfile "$minial_dockerfile" --no-cache --verbose --dry-run true 2>&1)
# then:
grep -qE "Building docker image" <<< "$out"
grep -qE "DRY_RUN: docker build .+ --no-cache" <<< "$out"
...
# when:
out=$(plain-sandbox --dockerfile "$minial_dockerfile" --rebuild --env-file no-such-file --allow-net --verbose true 2>&1) || status=$?
# then:
test "$status" -ne 0
